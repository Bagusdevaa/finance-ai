...
**Tech Stack:** Docker Compose v3.8, python:3.12-slim, node:22-alpine, pnpm 10.33.2 pinned via corepack (matches host version), caddy:2-alpine, postgres:16-alpine, qdrant:latest, GNU Make.
...
**NOT modified:** `docker-compose.yml`, `Caddyfile`, `Caddyfile.dev`, `backend/Dockerfile`, `.env`, `.env.example`.

**Modified during execution:** `frontend/package.json` — added `pnpm.onlyBuiltDependencies: ["unrs-resolver"]` block. pnpm 11.x treats ignored postinstall scripts as hard failure (was warning in pnpm 10). `unrs-resolver` is a transitive dep from `eslint-config-next` that builds a native binary at install time — letting it build gives ESLint the faster native resolver. This is the recommended pnpm 11 migration pattern, not a workaround.
...
# Enable corepack and pin pnpm to match host version exactly (10.33.2 as of 2026-05-28).
# Avoids version drift: lockfile was generated by pnpm 10 (host), and pnpm 11 has stricter
# postinstall script policy that requires lockfile-recorded approval — using pnpm 10 here
# stays consistent with how user installs locally.
...
services:
  ...
  backend:
    ...
    environment:
      ...
      # Force watchfiles polling — Mac VirtioFS bind mount doesn't reliably
      # emit file change events to Linux container kernel
      WATCHFILES_FORCE_POLLING: "true"
      # Override conftest's hardcoded localhost:5432 (fails inside container)
      # untuk point ke postgres service via compose network
      TEST_DATABASE_URL: postgresql+asyncpg://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/financeai_test
